...
//?????????????????????????????????????????????????????????????????????????????
function rewind(){
  time = myVideo1.currentTime; // VARIABLE DE TIEMPO ACTUAL
  if (looping && time > upperinterval){ // SI ESTA LOOPING Y EL TIME ES MAYOR QUE EL UPPERINTERVAL,
    time = lowerinterval; // ACTUALIZO VG TIME
